...
const MerlynsRest = new market('1012', 'Merlyns Rest')
...
const userInput = 
  `Please select a city by entering a number between 1 and 8:
...
8. ${marketList[7].marketName}
`
rl.question(userInput, (userInput) => {
  const selectedMarket = marketList[userInput -1];
  

  console.log(`Loading the Best Trades from ${selectedMarket.marketName} to the rest of the Albion Universe`);
  rl.close();
  startCalculating(selectedMarket);
});

function startCalculating(selectedMarket) {
  calculateArbitrageOpportunities(selectedMarket);
}
...
async function calculateArbitrageOpportunities(selectedMarket) {
  const batchSize = 100;
